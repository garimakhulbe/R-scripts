# Columns required to find LACE risc score

columns_to_include <- c("rln","diag_p","odiag1",
                    	"odiag2", "odiag3",
                    	"odiag4","odiag5","odiag6","odiag7",
                    	"odiag8","odiag9","odiag10",
                    	"odiag11","odiag12","odiag13","odiag14","odiag15",
                    	"odiag16","odiag17","odiag18",
                    	"odiag19","odiag20","odiag21"
                    	,"odiag22","odiag23","odiag24","los",
                    	"srcroute", "admtdate",
                    	"dschdate", "next_admit_date","thirtyday" )
CCS_CODE_COLUMN <- c("odiag1","odiag2", "odiag3",
                    	"odiag4","odiag5","odiag6","odiag7",
                    	"odiag8","odiag9","odiag10",
                    	"odiag11","odiag12","odiag13","odiag14","odiag15",
                    	"odiag16","odiag17","odiag18",
...
read_oshpd_data <- function(filename, admit_date, discharge_date) {
  data_frame <- readRDS(filename, header=T, na.strings="")
...
find_and_make_chf <- function(patient_data, ccs_to_morbid_file, out_file_name) {
  ...
  for (i in seq(nrow(patient_data))) {
	...
	codes <- patient[, CCS_CODE_COLUMN] # CCS_CODE_COLUMN (defined at top) is the name of columns for which we wish to find the comorbid names
	morbids <- find_chf(codes, ccs_to_morbid) # Given list of ccs code find their corresponding comorbidity name
 #   print (morbids)
...
thirtyday_data <- read_oshpd_data("testdata.seqLength12_version3.rds", "admtdate", "dschdate")
